...
class scheduleDAO extends baseDAO {
	...
	function GenerateCalendar() {
		...
		try {
			...
			foreach ( $sth->fetchAll () as $row ) {
				...
				// Event Title
				$title = sprintf ( __ ( '[%s to %s] %s scheduled on %s (Order: %d)' ), DateManager::getLocalDate ( Kit::ValidateParam ( 'FromDT', _INT ) ), DateManager::getLocalDate ( Kit::ValidateParam ( 'ToDT', _INT ) ), Kit::ValidateParam ( $row ['Campaign'], _STRING ), Kit::ValidateParam ( $row ['DisplayGroups'], _STRING ), Kit::ValidateParam ( $row ['DisplayOrder'], _INT ) );
	...
	function AddEventForm() {
		...
		$formFields ['general'] [] = FormManager::AddMultiCombo ( 'DisplayGroupIDs[]', __ ( 'Display' ), $displayGroupIds, array (
				'group' => $groups,
				'display' => $displays 
		), 'displaygroupid', 'displaygroup', __ ( 'Please select one or more displays / groups for this event to be shown on.' ), 'd', '', true, '', '', '', $optionGroups, array (
				array (
						'name' => 'data-live-search',
						'value' => "true" 
				),
				array (
						'name' => 'data-selected-text-format',
						'value' => "count > 4" 
				) 
		) );
		
		// Time controls
		$formFields ['general'] [] = FormManager::AddText ( 'starttimeControl', __ ( 'Start Time' ), NULL, __ ( 'Select the start time for this event' ), 's', 'required' );
		
		$formFields ['general'] [] = FormManager::AddText ( 'endtimeControl', __ ( 'End Time' ), NULL, __ ( 'Select the end time for this event' ), 'e', 'required' );
		...
		$formFields ['general'] [] = FormManager::AddCombo ( 'CampaignID', __ ( 'Layout / Campaign' ), NULL, array (
				'campaign' => $campaignOptions,
				'layout' => $layoutOptions 
		), 'id', 'value', __ ( 'Please select a Layout or Campaign for this Event to show' ), 'l', '', true, '', '', '', $optionGroups );
		
		$formFields ['general'] [] = FormManager::AddNumber ( 'DisplayOrder', __ ( 'Display Order' ), NULL, __ ( 'Please select the order this event should appear in relation to others when there is more than one event scheduled' ), 'o' );
		
		$formFields ['general'] [] = FormManager::AddCheckbox ( 'is_priority', __ ( 'Priority' ), NULL, __ ( 'Sets whether or not this event has priority. If set the event will be show in preference to other events.' ), 'p' );
		
		$formFields ['repeats'] [] = FormManager::AddCombo ( 'rec_type', __ ( 'Repeats' ), NULL, array (
				array (
						'id' => '',
						'name' => __ ( 'None' ) 
				),
		...
		), 'id', 'name', __ ( 'What type of repeat is required?' ), 'r' );
		
		$formFields ['repeats'] [] = FormManager::AddNumber ( 'rec_detail', __ ( 'Repeat every' ), NULL, __ ( 'How often does this event repeat?' ), 'o', '', 'repeat-control-group' );
		
		$formFields ['repeats'] [] = FormManager::AddText ( 'rec_rangeControl', __ ( 'Until' ), NULL, __ ( 'When should this event stop repeating?' ), 'u', '', 'repeat-control-group' );
		
		$formFields ['repeats'] [] = FormManager::AddHidden ( 'rec_range', NULL );
		
		// Set some field dependencies
		$response->AddFieldAction ( 'rec_type', 'init', '', array (
				'.repeat-control-group' => array (
						'display' => 'none' 
				) 
		) );
		$response->AddFieldAction ( 'rec_type', 'init', '', array (
		...
		$response->AddFieldAction ( 'rec_type', 'change', '', array (
				'.repeat-control-group' => array (
						'display' => 'none' 
				) 
		) );
	...
	function EditEventForm() {
		...
		$formFields ['general'] [] = FormManager::AddCombo ( 'CampaignID', __ ( 'Layout / Campaign' ), $campaignId, array (
				'campaign' => $campaignOptions,
				'layout' => $layoutOptions 
		), 'id', 'value', __ ( 'Please select a Layout or Campaign for this Event to show' ), 'l', '', true, '', '', '', $optionGroups );
		...
		$formFields ['repeats'] [] = FormManager::AddCombo ( 'rec_type', __ ( 'Repeats' ), $recType, array (
				array (
						'id' => '',
						'name' => __ ( 'None' ) 
				),
	...
	function DeleteForm() {
		...
		Theme::Set ( 'form_fields', array (
				FormManager::AddMessage ( __ ( 'Are you sure you want to delete this event from <b>all</b> displays? If you only want to delete this item from certain displays, please deselect the displays in the edit dialogue and click Save.' ) ) 
		) );
	...
	/**
	 * Remove the expired events from schedule
	 *
	 * {@inheritDoc}
	 *
	 * @see baseDAO::removePriorityEvent()
	 */
	public function removeExpiredEvent($expiredEventID) {
		// Check the token
		$db = & $this->db;
		$user = & $this->user;
		$response = new ResponseManager ();
		
		foreach ( $expiredEventID as &$ID ) {
			if ($ID == 0)
				echo '\nNo event selected.';
			...
			// Delete the entire schedule.
			if (! $scheduleObject->Delete ( $ID )) {
				trigger_error ( $scheduleObject->GetErrorMessage (), E_USER_ERROR );
			}
			// echo "\nThe Expired event has been Deleted.";
			$response->Respond ();
		}
	}
	...
	public function ScheduleNowForm() {
		...
		$formFields [] = FormManager::AddCombo ( 'CampaignID', __ ( 'Layout' ), $campaignId, array (
				'campaign' => $campaignOptions,
				'layout' => $layoutOptions 
		), 'id', 'value', __ ( 'Please select a Layout or Campaign for this Event to show' ), 'l', '', true, '', '', '', $optionGroups );
		...
		$formFields [] = FormManager::AddMultiCombo ( 'DisplayGroupIDs[]', __ ( 'Display' ), $displayGroupIds, array (
				'group' => $groups,
				'display' => $displays 
		), 'displaygroupid', 'displaygroup', __ ( 'Please select one or more displays / groups for this event to be shown on.' ), 'd', '', true, '', '', '', $optionGroups, array (
				array (
	...
	public function displayNow($keyCampaignIDs, $displayGroupID) {
		...
		$keyCampaignIds = $keyCampaignIDs;
		$displayGroupIds = array (
				$displayGroupID 
		);
		// Validate
		if (count($keyCampaignIDs) == 0)
			trigger_error ( __ ( 'No layout selected' ), E_USER_ERROR );
			// check that at least one display has been selected
		if ($displayGroupIds == '')
			trigger_error ( __ ( 'No displays selected' ), E_USER_ERROR );
		...
		// Querry lai UserID cua admin trong database theo hai bang User va Usertype
		$this->user->userid = 1;
		$fromDt = time ();
		if ($displayOrder <= $globalOrder)
			$displayOrder=-1;
		...
		foreach ($keyCampaignIds as &$cID){
			if (! $scheduleObject->AddNewSchedule ( $displayGroupIds, $fromDt, $toDt, $cID, '', '', '', $isPriority, $this->user->userid, $displayOrder-1 )) {
				//trigger_error ( , E_USER_ERROR );
				echo $scheduleObject->GetErrorMessage ();
			} else
				echo "\nThe new event for CampaignID: '$cID' has been Scheduled\n";
		}
		
		$response->Respond ();
	}
	
	public function querryDB() {
		require ("globalVar.php");
		...
		// Get campaignID corresponding with Gender
		$campaignID = null;
		$layoutName = null;
		
		// Querry the list of Layout in database
		$layouts = array ();
		$sql = "SELECT layout FROM layout";
		...
		if ($result->num_rows > 0) {
			...
			echo "\nThere is no layout in Database";
		}
		// Choose randomly layout and CampaignID from Database
		$keyLayouts = array ();
		$keyCampaignIDs = array ();
		
		foreach ( $layouts as &$layout ) {
			if (strpos ( $layout, $layoutKey ) !== false){
				$keyLayouts [] = $layout;
				$sql = "SELECT CampaignID FROM campaign WHERE Campaign='$layout'";
				$result = $conn->query ( $sql );
				if ($result->num_rows > 0) {
					$row = $result->fetch_assoc ();
					$campaignID = $row ["CampaignID"];
					$keyCampaignIDs[]=$campaignID;
				} else 
					echo "\nnonRecord for campaignID";
			}
			
		}
// 		$randomLayout = $keyLayouts [array_rand ( $keyLayouts, 1 )];
		
// 		// Querry from Campaign table
// 		$sql = "SELECT CampaignID FROM campaign WHERE Campaign='$randomLayout'";
// 		$result = $conn->query ( $sql );
// 		if ($result->num_rows > 0) {
// 			$row = $result->fetch_assoc ();
// 			$campaignID = $row ["CampaignID"];
...
// 		}
		...
		// _______________________________________________
		// Querry the ID of expired eventID
		$now = time ();
		$sql = "SELECT eventID FROM schedule WHERE ToDT < '$now' AND  DisplayGroupIDs='$displayGroupID'";
		$result = $conn->query ( $sql );
		$expiredEventID = array ();
		if ($result->num_rows > 0) {
			// Sử dụng vòng lặp while để lặp kết quả
			while ( $row = $result->fetch_assoc () ) {
				$expiredEventID [] = $row ["eventID"];
			}
		}
	...
	/*
	 * Get MIN of display order in Schedule table
	 */
	public function querryMINDisplayOrder() {
		require ("globalVar.php");
		$conn = new mysqli ( $dbhost, $dbuser, $dbpass, $dbname );
		// SQL connection checking
		if ($conn->connect_error) {
