...
class Schedule extends Data {
	 ...
	 * Add new schedule
	 */
	public function AddNewSchedule($displayGroupIDs, $fromDT, $toDT, $campaignId, $recType, $recDetail, $recToDT, $isPriority, $userID, $displayOrder = 0) {
		Debug::LogEntry ( 'audit', 'IN', 'Schedule', 'Add' );
...
// 				return $this->SetError ( 25001, __ ( 'No User Id Present' ) );
			...
			if ($recDetail == 0)
				...
				// make the displayid_list from the selected displays.
			$displayGroupIDList = implode ( ",", $displayGroupIDs );
			// $displayGroupIDList = $displayGroupIDs;
			// Parameters for the query
			$params = array (
					'campaignid' => $campaignId,
					'displaygroupids' => $displayGroupIDList,
	...
	public function Delete($eventID) {
		...
		try {
			...
			\Xibo\Helper\Log::audit ( 'Schedule', $eventID, 'Schedule Event Deleted', array (
					'eventId' => $eventID 
			) );
	...
	/**
	 * Adds a Schedule Detail record.
	 * This can optionally be linked to a Schedule Event record.
	 * 
	 * @return
	 *
	...
	public function deleteAllForUser($userId) {
		...
		$eventIds = array_map ( function ($element) {
